...
export async function GET(request) {
    try {
        ...
        if (searchParams.get('lineUserId'))
            filter.lineUserId = searchParams.get('lineUserId');
        if (searchParams.get('characterId'))
            filter.characterId = searchParams.get('characterId');
...
export async function POST(request) {
    try {
        ...
        const conversation = await createOne(Conversation, Object.assign(Object.assign({}, body), { timestamp: new Date() }));
